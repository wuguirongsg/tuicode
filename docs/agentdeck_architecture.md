...
AgentDeck 的主路径不是接管智能体，而是承载成熟 CLI 智能体：智能体继续在 PTY 中运行自己的完整交互体系，AgentDeck 负责把它们工作的项目状态可视化、同步和组织起来。
...
- 多个智能体可以并行会话，互不干扰
- 智能体或用户在项目中产生的文件/Git 变化会被 AgentDeck 自动感知并反馈到文件树、编辑器和 Git 面板
- 深度能力（上下文注入、工具调用拦截、MCP 集成）是后续增强，不作为 MVP 主路径

### 2.2 上下文即产品

AgentDeck 的核心引擎是**工作区状态聚合器**：它持续维护项目的"当前态"，包括当前活动文件、选区、最近外部文件变化、Git 状态和终端活动。MVP 阶段它主要服务 UI 同步；后续深度集成阶段再按需打包给智能体。
...
AgentDeck 的默认集成方式是 **PTY 承载**。Claude Code、Codex、Aider 已经是完整的 CLI 产品，它们自带上下文管理、权限提示、工具执行和会话历史。MVP 不重复实现这些能力，而是让它们稳定运行在浮窗中，并通过文件监视和 Git 轮询感知它们对项目造成的结果。

这让 AgentDeck 的第一性价值从"替代智能体交互"转为"提供智能体工作台"：

- 用户仍按熟悉方式使用 Claude Code / Codex
- AgentDeck 管理多个智能体窗口、编辑器、文件树、运行终端和 Git 状态
...
| MCP Bridge | Phase 3+ | 让外部智能体调用 AgentDeck 的文件、Git、上下文能力 |
...
MVP 不在 AgentDeck 内二次审批智能体工具调用。Claude Code 等智能体已有自己的权限提示和配置体系，重复接管会带来额外复杂度和协议耦合。后续如果进入 Adapter / MCP 模式，再提供统一审批流程：
...
整个过程里，AgentDeck 不接管 Claude Code 的内部工具流，但能感知它对工作区造成的结果，用户视觉焦点始终在 TuiCode 内。
