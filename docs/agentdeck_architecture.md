...
**为终端编程智能体（Claude Code、Codex、Aider 等）设计的统一 TUI 工作台**，提供 IDE 级的编辑、文件、终端、Git、上下文管理体验，让"人 + 多个 AI 智能体"在同一个终端环境里协同工作。
...
| 核心价值 | 把"编辑器 + 文件管理 + 多终端 + Git + 智能体会话"整合为一致体验 |
...
主流 IDE 把 AI 当成插件（Copilot 浮窗、Cursor 侧栏）。AgentDeck 反过来：**编辑器、文件树、终端都是为了让智能体更好地工作而存在**。

这意味着：

- 智能体可以原生读取当前编辑器状态、光标位置、选区、最近变更
- 智能体可以申请打开/关闭文件、运行命令、提交 Git，由用户审批
- 多个智能体可以并行会话，互不干扰
- 切换智能体不影响项目上下文

### 2.2 上下文即产品

智能体的输出质量本质上由上下文质量决定。AgentDeck 的核心引擎是**上下文聚合器**：它持续维护项目的"当前态"，按需打包给智能体。

### 2.3 协议驱动，不绑定具体智能体

通过抽象的 **Agent Adapter Protocol**，新增智能体只需实现一个适配器，无需修改宿主。短期接入 Claude Code、Codex、Aider；长期对接任意符合 MCP 协议的智能体或自建模型。
...
这条原则让"灵活"和"可预测"在同一界面共存，是 AgentDeck 区别于纯 tmux 风格和纯桌面浮窗的关键设计判断。
...
1. 用户在 AgentDeck 中打开项目，文件树展开到目标模块
2. 在编辑器里查看现有代码，选中要重构的函数
3. 按 `Ctrl+Shift+A` 唤出 Agent 面板，选择 Claude Code
4. Agent 自动获取选区代码、文件路径、相关 import 作为上下文
5. 用户提示"将这个函数重构为更小的单元"
6. Agent 输出方案，用户在编辑器侧实时看到 diff 预览
7. 一键应用或拒绝，应用后 Git 状态面板自动刷新
...
1. SSH 到远程机器，启动 AgentDeck
2. 在嵌入终端里启动一个长时间运行的训练任务
3. 同时与智能体对话，让它修改其他模块的代码
4. 切到 Git 面板提交另一组变更，全程不离开 AgentDeck
...
├──────────────┴──────────────┴─────────────┴───────────────┤
│              Agent 面板（核心创新区）                       │
│  • 多 Agent 会话管理   • 流式输出   • 工具调用审批           │
│  • Agent 切换         • 历史记录   • Diff 预览              │
├───────────────────────────────────────────────────────────┤
│              上下文聚合引擎                                 │
│  • 当前编辑状态  • 项目结构  • Git 状态  • 命令历史          │
├───────────────────────────────────────────────────────────┤
│              Agent Adapter 协议层                          │
│  Claude Code 适配  │  Codex 适配  │  Aider 适配  │  MCP    │
├───────────────────────────────────────────────────────────┤
│              基础设施层                                     │
...
| Agent 面板 | P0 | ✓ | 至少接入 Claude Code |
| 上下文聚合器 | P0 | ✓ | 基础版本 |
| Git 状态面板 | P1 |   | 状态、diff、commit |
| 多 Agent 适配 | P1 |   | 同时接入 Codex、Aider |
...
- **主进程** —— Textual 事件循环
- **PTY 子进程** —— 每个嵌入式终端独立 PTY
- **Agent 子进程** —— 每个智能体会话独立子进程（通过 stdio 或 PTY 通信）
- **后台协程** —— 文件监视、Git 状态轮询、上下文索引

```
AgentDeck Main Process (Textual asyncio loop)
├── PTY: bash session #1
├── PTY: bash session #2
├── Subprocess: claude-code (via stdio)
├── Subprocess: codex (via PTY)
├── Coroutine: file watcher (watchdog)
├── Coroutine: git status poller
└── Coroutine: context indexer
```
...
模块订阅自己关心的事件，上下文聚合器订阅几乎所有事件以维护全局状态。
...
**MVP 直接使用 Textual 0.47+ 的 TextArea 组件**，它已经内置：
- 语法高亮（基于 tree-sitter）
- 行号、当前行高亮
- 多语言支持
- 复制粘贴
...
### 6.4 Agent Adapter 协议层（核心创新）

这是 AgentDeck 区别于其他 TUI 工具的关键。设计目标：**一个协议接入所有智能体**。
...
| 智能体 | 通信方式 | 实现路径 |
|---|---|---|
| Claude Code | CLI + stdio | 子进程 + 解析输出流 |
| Codex CLI | CLI + stdio | 类似 |
| Aider | CLI + PTY | 启动 aider，PTY 包装 |
| OpenAI 直连 | HTTP API | 直接调用 |
| Anthropic 直连 | HTTP API | 直接调用 |
| MCP Server | MCP 协议 | 标准 MCP 客户端 |

#### 6.4.3 工具调用审批工作流

智能体想要执行操作（运行命令、修改文件、提交代码）时，统一走审批流程：
...
聚合器维护一个**实时更新的项目快照**，对所有 Adapter 提供按需查询接口。
...
Git 是开发流程核心，但 AgentDeck 不重复造轮子，**直接复用 lazygit 思路**：通过 `GitPython` 或 `pygit2` 操作仓库，UI 借鉴 lazygit。
...
中后期，AgentDeck 既可以是 **MCP 客户端**（接入外部 MCP 服务），也可以暴露为 **MCP 服务器**（让其他智能体把 AgentDeck 的能力当作工具用）。
...
这一层将 AgentDeck 变成一个**通用的 AI 编程工作站接口**，任何 MCP 兼容的智能体都能驾驭它。

---

## 7. 数据流示例

**场景：用户在 Claude Code 中请求修改文件**

```
1. 用户在 Agent 面板输入: "把 utils.py 的 parse_date 改成接受时区参数"
                                    ↓
2. 上下文聚合器打包:
   - utils.py 完整内容
   - parse_date 的所有调用方（从代码索引中查）
   - 最近的 git diff
                                    ↓
3. Claude Code Adapter 通过 stdio 发送 prompt + context
                                    ↓
4. Adapter 流式接收响应，转为 AgentMessage 事件
                                    ↓
5. Agent 面板渲染响应（含代码块）
                                    ↓
6. Claude Code 请求工具: write_file(utils.py, new_content)
                                    ↓
7. Adapter 发出 ToolCallRequested 事件
                                    ↓
8. 编辑器侧弹出 Diff 预览面板（红绿对比）
                                    ↓
9. 用户按 y 批准
                                    ↓
10. 文件写入磁盘 → FileModified 事件
                                    ↓
11. 编辑器刷新、Git 状态面板刷新、聚合器更新
                                    ↓
12. ToolResult 返回给 Claude Code
                                    ↓
13. Claude Code 继续输出 "已修改，同时更新了 3 处调用方"
```

整个过程一气呵成，用户视觉焦点始终在 AgentDeck 内。
...
```python
class AgentDeckPlugin(Protocol):
    name: str
    version: str

    def on_load(self, app: AgentDeckApp): ...
...
采用 TOML 配置文件，路径：`~/.config/agentdeck/config.toml`
...
- 混合布局窗口管理器
  ...
  - 右侧固定工具栏（先实现 files Tab，git Tab 可占位）
...
**可交付**：在 AgentDeck 中用 Claude Code 编辑一个 Python 项目，完成 read/write/run_command 流程。

### Phase 2 — 多智能体 + Git + 布局打磨（4-6 周）

- Agent Adapter 协议正式抽象
- 加入 Codex、Aider 适配器
- Git 状态面板（status / diff / commit）
- 底部运行终端多标签
...
Python 性能上限低，但 AgentDeck 的瓶颈是网络 IO 和终端 IO，而非 CPU。选 Python 是为了：1）AI 生态最成熟；2）社区贡献门槛低；3）作者熟悉。
...
| 维度 | VSCode | Cursor | Aider | Claude Code | Helix | **AgentDeck** |
|---|---|---|---|---|---|---|
| 终端原生 | ✗ | ✗ | ✓ | ✓ | ✓ | ✓ |
| GUI 级编辑体验 | ✓✓ | ✓✓ | ✗ | ✗ | ✓ | ✓ |
| 文件管理器 | ✓✓ | ✓✓ | ✗ | ✗ | △ | ✓ |
...
AgentDeck 占据的位置是：**"我想在终端里用 AI 编程，但希望有 IDE 级的文件 / 编辑 / Git 体验，并且能并行管理多个智能体"**。
...
- **开源核心** —— 编辑器、文件管理、终端、基础 Agent 适配，永久免费
...
1. **技术验证 spike**（1 周）—— 用最小代码验证 PTY + pyte + Textual 集成可行性
2. **MVP 范围冻结**（1 天）—— 把 Phase 1 的功能清单严格定义、写成 issue
3. **Claude Code stdio 协议梳理**（2 天）—— 摸清 Claude Code CLI 的 stdio 接口细节
4. **Textual TextArea 能力评估**（2 天）—— 实测它能否承担编辑器主体
...
- 是否需要支持远程模式（AgentDeck 服务端 + 瘦客户端）？
- 是否对接 OpenAI Realtime API 之类的语音模态？
- 上下文聚合器需不需要做向量索引（vs 纯文本检索）？
